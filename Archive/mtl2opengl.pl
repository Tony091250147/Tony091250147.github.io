...
sub handleArguments() 
{
	...
	unless($errorInOptions) 
	{
		...
		$outPositions = $dirOBJ . $fileOBJ . "positions.txt";
		$outNormals = $dirOBJ . $fileOBJ . "normals.txt";
		$outTexels = $dirOBJ . $fileOBJ . "texels.txt";
...
sub loadDataOBJ 
{
	...
	open (OUTFILEPOS, ">$outPositions") 
	  || die "Can't create file '$outFilenameOBJ' ... exiting\n"; 
	open (OUTFILENOM, ">$outNormals")
	  || die "Can't create file '$outFilenameOBJ' ... exiting\n";
	open (OUTFILETEX, ">$outTexels")
	  || die "Can't create file '$outFilenameOBJ' ... exiting\n";
	...
	while ($line = <INFILEOBJ>) 
	{
	  	...
	  	if ($line =~ /v\s+.*/)
	  	{
	    	...
	    	print OUTFILEPOS "$xcoords[$numVerts],$ycoords[$numVerts],$zcoords[$numVerts],\n";
			#print "'$numVerts' '$x'\n";
			#print "'$numVerts' '$y'\n";
			#print "'$numVerts' '$z'\n";
	    	$numVerts++;
	  	...
	  	if ($line =~ /vt\s+.*/)
	  	{
	    	...
	    	print OUTFILETEX "$tx[$numTexture],$ty[$numTexture],\n";
	    	$numTexture++;
	  	}
	  	...
	  	if ($line =~ /vn\s+.*/)
	  	{
			...
			print OUTFILENOM "$nx[$numNormals],$ny[$numNormals],$nz[$numNormals],\n";
	    	$numNormals++;
	  	}
